...
v0.5.2。2D 73 / 3D 22 / zumen 連携 5 / game 連携 15 / Web 連携 11 / 認知 21 / 集約 15 / 町 80 / 地形 16 / 植生 13 / 認知地図 14 / 樹形 45 / 塊り 170 / 文書の実行 11 / 文書の整合 6 / デモ smoke 5 の計 522 テスト。\n\n文書に載せたコードは `docs/examples.mjs` が実際に実行し、規則と API の一覧は `docs/check-docs.mjs` が実装と突き合わせる。**文書が嘘をつくのを機械で止めている。**
...
| `haichi-engine/tree` | **木の形をフラクタルで作る。** 幹から枝、枝からまた枝。種の違いは分岐角・子の数・縮み率・上向き・幹の割合の 5 つの数字でほぼ言い尽くせる。9 種（広葉樹・杉・松・欅・街路樹・枝垂れ・椰子・竹・低木） |
| `haichi-engine/building` | **建物を箱 1 個にしない。** L 字・コの字・ロの字・雁行・基壇＋塔・段状・翼屋の 8 型と、上に載るもの（塔屋・給水塔・換気塔・煙突・外階段・袖看板・ベランダ） |
...
規則は P101–P106（塊り）、B101–B105（樹形）、T101–T107（町）、R101–R105（地形）、G101–G105（植生）、M101–M104（認知地図）。
...
建築基準法も守らせているのは建物どうしの距離ではなく接道（幅 4 m 以上の道路に 2 m 以上）のほう。

なので **不変条件は接道（T107）に置き、隙間は用途で選ぶ**。

| realism | 隙間 | 見え方 | 向いている用途 |
|---|---|---|---|
| `real` | 0.6 m | 肩がぶつかる路地。下町・宿場町・スラム | 実物に寄せたいとき |
| `walkable`（既定） | 5 m | 建物の間も歩ける | 街を歩き回るのが目的のとき |

どちらでも **全戸が幅 4 m 以上の道に接する**（T107）。まばらに建てる `scatter` では
一軒ずつ私道を引き、沿道が埋まって外へ押し出すときは裏通りを一緒に引く。
これを入れる前は実測で 126/150 が道に接しておらず、最遠で 703 m 離れていた。

`mapability()` は「見た目の良し悪し」ではなく **後で思い出せるか** を測る。
同じ型の地区を並べると M101 が指摘し、地区の特徴が近すぎると M103 が組を挙げる。
`adjacency` を省略すると、各軸の矩形間隔の最大値が両地区の `max(w,d)` の小さいほうの35%未満なら隣接と判定する。地区の配列順によらず同じ閾値を使い、明示した `adjacency`（空配列を含む）を優先する。
