...
既定で**入力の図形そのものを避ける**。他のラベルだけでなく、駅の丸やカードの上に載らない。

```js
placeLabels(shapes, {
  obstacles: [{ id: 'toolbar', x: 400, y: 20, w: 800, h: 40 }],  // ここには置くな
  edges: [{ from: 'a', to: 'b' }],   // この線を横切るな
  bounds: { x: 400, y: 300, w: 800, h: 600 },  // この外へ出すな
  avoidShapes: false,                // 図形を避けない（従来の挙動）
});
```

- **自分を包んでいる図形は避けない**（入れ子で置かれているのだから、その上に載るのは当たり前）
- `obstacles` は明示指定なので、包んでいようが必ず避ける
- 置けなかったものは `hidden: true` と `why`（何に阻まれたか）を返す
