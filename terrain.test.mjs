...
// 幅または奥行きが 0 の敷地でも、測定を止めず有限な結果を返す
{
  ...
  const validReport = (report) =>
    ['flatInside', 'relief', 'ridgeRatio'].every((key) => Number.isFinite(report[key])) &&
    ['riverCrosses', 'continuous'].every((key) => typeof report[key] === 'boolean') &&
    Object.keys(report).sort().join(',') === 'continuous,flatInside,relief,ridgeRatio,riverCrosses';
  ...
  ok('幅 0 の敷地を5指標で測れる', validReport(zeroWidth));
  ok('奥行き 0 の敷地を5指標で測れる', validReport(zeroDepth));

  const nonZero = terrain({ span: 6000, res: 129, seed: 3, river: false });
  const measured = measureTerrain(nonZero, { flat: { x0: -100, x1: 100, z0: -100, z1: 100 } });
  ok('非ゼロ範囲の既存の測定結果を変えない', measured.flatInside === 6.393056731047469,
    `flatInside ${measured.flatInside}`);
}
